...
function shouldPreserveTransitStatus(status) {
    return ["reset", "manual_missing_out"].includes(String(status || "").trim().toLowerCase());
}
...
function syncTransitForPulse(db, pulseRow) {
    ...
    if (isEntrySensor(sensorId)) {
        ...
        const nextStatus = shouldPreserveTransitStatus(existingTransit?.status)
            ? existingTransit.status
...
function listOvenTransitsForBatch(dbPath, { batchId = null, limit = 200 } = {}) {
    const db = getDb(dbPath);
    const normalizedBatchId = Number(batchId);
    const normalizedLimit = Math.max(1, Math.min(Number(limit) || 200, 500));

    if (!Number.isInteger(normalizedBatchId) || normalizedBatchId <= 0) {
        throw new Error("Brakuje ID wejscia MES do analizy transitow.");
    }

    const batch = selectBatchRowById(db, normalizedBatchId);
    if (!batch) {
        throw new Error("Nie znaleziono wejscia MES do analizy transitow.");
    }
    ...
    const rows = db.prepare(`
        SELECT
            ...
            t.pulse_in_id AS pulseInId,
            t.pulse_out_id AS pulseOutId,
            t.batch_id_in AS batchIdIn,
            t.batch_id_out AS batchIdOut,
            t.entered_at AS enteredAt,
            t.exited_at AS exitedAt,
            t.duration_seconds AS durationSeconds,
            t.status,
            pin.ts AS pulseInTs,
            pout.ts AS pulseOutTs,
            pin.batch_id AS pulseInBatchId,
            pout.batch_id AS pulseOutBatchId,
            pin.sensor_id AS pulseInSensorId,
            pout.sensor_id AS pulseOutSensorId,
            pin.payload_json AS pulseInPayloadJson,
            pout.payload_json AS pulseOutPayloadJson
        FROM mes_oven_transits t
        LEFT JOIN oven_pulses pin
          ON pin.id = t.pulse_in_id
        LEFT JOIN oven_pulses pout
          ON pout.id = t.pulse_out_id
        WHERE t.batch_id_in = ?
           OR (t.batch_id_in IS NULL AND t.batch_id_out = ?)
        ORDER BY COALESCE(t.entered_at, t.exited_at) DESC, t.id DESC
        LIMIT ?
    `).all(normalizedBatchId, normalizedBatchId, normalizedLimit);

    const stats = rows.reduce((accumulator, row) => {
        const hasIn = Boolean(row.pulseInId);
        const hasOut = Boolean(row.pulseOutId);

        if (hasIn) {
            accumulator.inputCount += 1;
        }

        if (hasOut || row.status === "manual_missing_out") {
            accumulator.outputCount += 1;
        }

        if (row.status === "manual_missing_out") {
            accumulator.manualCloseCount += 1;
            return accumulator;
        }

        if (row.status === "reset") {
            accumulator.resetCount += 1;
            return accumulator;
        }
        ...
        if (hasIn) {
            accumulator.missingOutCount += 1;
            return accumulator;
        }

        if (hasOut) {
            accumulator.orphanOutCount += 1;
        }
    ...
    return {
        ...
        stats,
        transits: rows.map((row) => ({
            id: row.id,
            deviceId: row.deviceId,
            sessionId: row.sessionId,
            pulseInId: row.pulseInId,
            pulseOutId: row.pulseOutId,
            batchIdIn: row.batchIdIn,
            batchIdOut: row.batchIdOut,
            enteredAt: row.enteredAt || row.pulseInTs || null,
            exitedAt: row.exitedAt || row.pulseOutTs || null,
            durationSeconds: row.durationSeconds === null || row.durationSeconds === undefined
                ? null
                : Number(row.durationSeconds),
            status: row.status || null,
            pulseInBatchId: row.pulseInBatchId || null,
            pulseOutBatchId: row.pulseOutBatchId || null,
            pulseInSensorId: row.pulseInSensorId || null,
            pulseOutSensorId: row.pulseOutSensorId || null,
            pulseInPayloadJson: row.pulseInPayloadJson || null,
            pulseOutPayloadJson: row.pulseOutPayloadJson || null,
        })),
    };
}
...
function aggregateTransitsByAttributionBatchIds(db, batchIds = []) {
    ...
    const row = db.prepare(`
        SELECT
            COUNT(CASE WHEN pulse_out_id IS NOT NULL OR status = 'manual_missing_out' THEN 1 END) AS outputCount,
...
function manuallyCloseOvenTransit(dbPath, { transitId = null, operator = "", reason = "missing_out_manual" } = {}) {
    const db = getDb(dbPath);
    const normalizedTransitId = Number(transitId);

    if (!Number.isInteger(normalizedTransitId) || normalizedTransitId <= 0) {
        throw new Error("Brakuje ID przejscia MES do recznego domkniecia.");
    }
    ...
    const transit = db.prepare(`
        ...
        WHERE id = ?
        LIMIT 1
    `).get(normalizedTransitId) || null;

    if (!transit) {
        throw new Error("Nie znaleziono przejscia MES do recznego domkniecia.");
    }

    const normalizedStatus = String(transit.status || "").trim().toLowerCase();
    if (!transit.pulseInId || transit.pulseOutId || !["open", "reset"].includes(normalizedStatus)) {
        throw new Error("Tylko przejscie bez OUT mozna domknac recznie.");
    }

    const now = new Date().toISOString();
    db.prepare(`
        UPDATE mes_oven_transits
        SET batch_id_out = COALESCE(batch_id_out, batch_id_in),
            status = 'manual_missing_out',
            updated_at = ?
        WHERE id = ?
    `).run(now, normalizedTransitId);

    const updatedTransit = db.prepare(`
        SELECT
            ${TRANSIT_SELECT_FIELDS}
        FROM mes_oven_transits
        WHERE id = ?
    ...
    const batch = updatedTransit?.batchIdIn
        ? hydrateBatch(db, selectBatchRowById(db, updatedTransit.batchIdIn))
        : null;

    return {
        transit: updatedTransit,
        batch,
        operator: toNullableText(operator),
        reason: toNullableText(reason) || "missing_out_manual",
        closedAt: now,
    };
}
