...
function selectOpenTransitRowsByDevice(db, deviceId) {
    return db.prepare(`
        SELECT
            id,
            device_id AS deviceId,
...
function listAttributedOvenPulsesForBatch(db, batchId, limit) {
    ...
    return db.prepare(`
        SELECT id, deviceId, batchId, sensorId, ts, payloadJson
        FROM (
            SELECT
                p.id AS id,
                p.device_id AS deviceId,
                p.batch_id AS batchId,
                p.sensor_id AS sensorId,
                p.ts AS ts,
...
function resetOpenOvenTransits(
    ...
    const now = new Date().toISOString();
    const openTransits = selectOpenTransitRowsByDevice(db, normalizedDeviceId);

    if (!openTransits.length) {
        return {
