...
function initializeSchema(db) {
    db.exec(`
        ...
        CREATE TABLE IF NOT EXISTS zapotrzebowanie_report_notes (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            code TEXT NOT NULL,
        ...
        CREATE INDEX IF NOT EXISTS idx_zapotrzebowanie_report_notes_code
            ON zapotrzebowanie_report_notes(code);
    `);
}
...
function getReportNotesMap(dbPath) {
    const db = getDatabase(dbPath);
    const rows = db.prepare(`
        SELECT
            id,
            ...
            changed_by AS changedBy,
            updated_at AS updatedAt
        FROM zapotrzebowanie_report_notes
    `).all();

    return new Map(rows.map((row) => [
        buildReportNoteKey(row.code, row.rodzaj),
        {
            id: Number(row.id),
            code: row.code || "",
            rodzaj: row.rodzaj || "",
...
function upsertReportNote({ dbPath, code, rodzaj, note, changedBy }) {
    const db = getDatabase(dbPath);
    const normalizedCode = String(code || "").trim().toUpperCase();
    const normalizedRodzaj = String(rodzaj || "").trim().toUpperCase();
    const normalizedNote = String(note ?? "").trim();
    const updatedAt = new Date().toISOString();

    if (!normalizedCode) {
        throw new Error("Brakuje poprawnego kodu dla uwagi raportu.");
    }
    if (!normalizedRodzaj) {
        throw new Error("Brakuje poprawnego rodzaju dla uwagi raportu.");
    }

    const previous = db.prepare(`
        SELECT id, note
        FROM zapotrzebowanie_report_notes
        WHERE code = ? AND rodzaj = ?
    `).get(normalizedCode, normalizedRodzaj);

    db.prepare(`
        INSERT INTO zapotrzebowanie_report_notes (
            code,
            rodzaj,
            note,
            changed_by,
        ...
        VALUES (?, ?, ?, ?, ?)
        ON CONFLICT(code, rodzaj) DO UPDATE SET
            note = excluded.note,
            changed_by = excluded.changed_by,
            updated_at = excluded.updated_at
    `).run(
